...
const getOneClass = (classList: CourseInfo[][], classNbr: number, quarter: string, courseQtr: number, numCourses: number) => {
...
const insertInOrder = (classList: CourseInfo[][], courseToAdd: CourseInfo[]) => {
...
interface PopulateDiv {
  classList?: CourseInfo[][];
  (classList: CourseInfo[][]): void;
};

const populateDiv: PopulateDiv = (classList: CourseInfo[][]) => {
...
const getClassPage = async (classNbr: number, courseQtr: number): Promise<string> => {
...
const getCachedData = async (courseQtr: number): Promise<CachedData> => {
...
const setCachedData = async (courseQtr: number, classList: CourseInfo[][]) => {
...
const displayRaw = (row: number, classList: CourseInfo[][]) => {
  // console.log(classList[row][0].raw);
  document.querySelector<HTMLElement>("#raw-data")!.style.display = 'inline'
  const innerEl = document.querySelector<HTMLElement>('#raw-data-inner')!;
  const nameEl = document.querySelector<HTMLElement>('#raw-data-name')!;
...
const convertCourseForAlphabetizing = (courseCode: string): string => {
...
const jsonToTable = (jsonData: string, level: number): string => {
  return "";    
}
